...
const BALL_RADIUS = 15; // DEFINED HERE
const BALL_COLOR = '#FFDE00'; // DEFINED HERE
...
const HEAD_RADIUS = 15; // For player
const BODY_WIDTH = 25;
...
function createPlayer(x, y, teamColor, isTeam1, inputKey) {
    ...
    const neckConstraint = Matter.Constraint.create({ bodyA: head, bodyB: playerBody, pointA: { x: 0, y: HEAD_RADIUS * 0.5 }, pointB: { x: 0, y: -BODY_HEIGHT / 2 }, length: 5, stiffness: 0.7, damping: 0.2, render: constraintRenderOptions });
    const leftHipConstraint = Matter.Constraint.create({ bodyA: playerBody, bodyB: leftLeg, pointA: { x: -BODY_WIDTH / 2 * 0.7, y: BODY_HEIGHT / 2 * 0.9 }, pointB: { x: 0, y: -LEG_HEIGHT / 2 * 0.9 }, length: 10, stiffness: 0.6, damping: 0.1, render: constraintRenderOptions });
    const rightHipConstraint = Matter.Constraint.create({ bodyA: playerBody, bodyB: rightLeg, pointA: { x: BODY_WIDTH / 2 * 0.7, y: BODY_HEIGHT / 2 * 0.9 }, pointB: { x: 0, y: -LEG_HEIGHT / 2 * 0.9 }, length: 10, stiffness: 0.6, damping: 0.1, render: constraintRenderOptions });
