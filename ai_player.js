...
let CANVAS_WIDTH = window.CANVAS_WIDTH || 800;
let CANVAS_HEIGHT = window.CANVAS_HEIGHT || 600;
...
let GOAL_HEIGHT = window.GOAL_HEIGHT || 120;
...
function determineAiState(ballPos, playerPos, halfX, ballVel) {
    ...
    if (ballInAiHalf && ballNearPlayerX && ballNearPlayerY) {
        if (ballPos.y < GROUND_Y - BALL_RADIUS * 0.5 &&
            ballAbovePlayer &&
            ballBelowPlayerHead &&
            (ballVel.x > -0.5 || playerPos.x < ballPos.x)) {
...
function handleIdleState(playerPos) {
    let targetX = CANVAS_WIDTH * 0.75; // موقعیت دفاعی عادی
    moveHorizontally(playerPos, targetX, MOVE_FORCE * 0.5); // سرعت عادی
}
